...
public class Skeleton extends Entity {
	...
	public void addPlatformRotatingCenter( Platform platform ) {
		...
		// center of platform
		RevoluteJoint joint = new RevoluteJointBuilder( world ).skeleton( this )
				.bodyB( platform ).build( );
	 ...
	 * @param platform
	 */
	public void addPlatformRotatingCenterWithRot( Platform platform,
			float rotSpeedInMeters ) {
		// Default values of the builder will allow rotation with anchor at
		// center of platform
		RevoluteJoint joint = new RevoluteJointBuilder( world ).skeleton( this )
				.bodyB( platform ).motor( true ).motorSpeed( rotSpeedInMeters )
				.build( );
	...
	public void addPlatformFixed( Platform platform ) {
		RevoluteJoint joint = new RevoluteJointBuilder( world ).skeleton( this )
				.bodyB( platform ).limit( true ).lower( 0 ).upper( 0 ).build( );
		addBoneAndJoint( platform, joint );
	}
	 ...
	 * 
	 * @param platform
	 *            - add platform that has structure screws already
	 */
	...
	public void addStrippedScrew( StrippedScrew ss ) {
		RevoluteJoint joint = new RevoluteJointBuilder( world ).skeleton( this )
				.bodyB( ss ).limit( true ).lower( 0 ).upper( 0 ).build( );
	...
	public void translate( float x, float y ) {
		body.setTransform( body.getTransform( ).getPosition( ).add( x, y ),
