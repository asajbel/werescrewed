...
public class PistonTweenMover extends TweenMover implements IMover {
	...
	public PistonTweenMover( Platform pistonPlatform, Vector2 pistonDestination, float timeUp, float timeDown, float delayUp, float delayDown, float delayOnce ) {
		...
		this.tween = Tween.to( pistonPlatform, PlatformAccessor.LOCAL_POS_XY, this.timeUp )
	...
	public void move( float deltaTime, Body body ) {
		...
		if ( tween.isFinished( ) ){
			...
			tween.kill( );
			if ( isGoingUp ){
				tween = Tween.to( pistonPlatform, PlatformAccessor.LOCAL_POS_XY, this.timeUp )
						 	 .target( this.pistonDestination.x, this.pistonDestination.y )
						 	 .delay( this.delayUp )
						 	 .start();
			}else{
				tween = Tween.to( pistonPlatform, PlatformAccessor.LOCAL_POS_XY, this.timeDown )
					 	 .target( 0f, 0f )
