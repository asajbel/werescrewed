...
public class ParticleEmitter {
	...
	public void update( float delta ) {
		...
		for ( int i = 0, n = active.length; i < n; i++ ) {
			if ( active[ i ]
					&& !updateParticle( particles[ i ], delta, deltaMillis ) ) {
				active[ i ] = false;
				activeCount--;
			}
		}
		...
		float emissionTime = emission + emissionDiff
				* emissionValue.getScale( durationTimer / ( float ) duration );
		if ( emissionTime > 0 ) {
			emissionTime = 1000 / emissionTime;
			if ( emissionDelta >= emissionTime ) {
				int emitCount = ( int ) ( emissionDelta / emissionTime );
				emitCount = Math
						.min( emitCount, maxParticleCount - activeCount );
	...
	private void activateParticle( int index ) {
		...
		particle.currentLife = particle.life = life
				+ ( int ) ( lifeDiff * lifeValue.getScale( percent ) );
		...
		if ( ( updateFlags & UPDATE_ANGLE ) == 0 ) {
			angle = particle.angle + particle.angleDiff
					* angleValue.getScale( 0 );
		...
		particle.setScale( particle.scale + particle.scaleDiff
				* scaleValue.getScale( 0 ) );
		...
		if ( rotationValue.active ) {
			...
			float rotation = particle.rotation + particle.rotationDiff
		...
		particle.transparencyDiff = transparencyValue.newHighValue( )
		...
		case square: {
			float width = spawnWidth
					+ ( spawnWidthDiff * spawnWidthValue.getScale( percent ) );
			float height = spawnHeight
					+ ( spawnHeightDiff * spawnHeightValue.getScale( percent ) );
		...
		particle.setBounds( x - spriteWidth / 2, y - spriteHeight / 2,
				spriteWidth, spriteHeight );

		int offsetTime = ( int ) ( lifeOffset + lifeOffsetDiff
				* lifeOffsetValue.getScale( percent ) );
		if ( offsetTime > 0 ) {
	...
	private boolean updateParticle( Particle particle, float delta,
			int deltaMillis ) {
		...
		if ( ( updateFlags & UPDATE_SCALE ) != 0 )
			particle.setScale( particle.scale + particle.scaleDiff
					* scaleValue.getScale( percent ) );

		if ( ( updateFlags & UPDATE_VELOCITY ) != 0 ) {
			float velocity = ( particle.velocity + particle.velocityDiff
					* velocityValue.getScale( percent ) )
					* delta;

			float velocityX, velocityY;
			if ( ( updateFlags & UPDATE_ANGLE ) != 0 ) {
				float angle = particle.angle + particle.angleDiff
						* angleValue.getScale( percent );
				velocityX = velocity * MathUtils.cosDeg( angle );
				velocityY = velocity * MathUtils.sinDeg( angle );
				if ( ( updateFlags & UPDATE_ROTATION ) != 0 ) {
					float rotation = particle.rotation + particle.rotationDiff
							* rotationValue.getScale( percent );
					if ( aligned )
						rotation += angle;
					particle.setRotation( rotation );
				}
			...
			if ( ( updateFlags & UPDATE_WIND ) != 0 )
				velocityX += ( particle.wind + particle.windDiff
						* windValue.getScale( percent ) )
						* delta;

			if ( ( updateFlags & UPDATE_GRAVITY ) != 0 )
				velocityY += ( particle.gravity + particle.gravityDiff
						* gravityValue.getScale( percent ) )
						* delta;

			particle.translate( velocityX, velocityY );
		} else {
			if ( ( updateFlags & UPDATE_ROTATION ) != 0 )
				particle.setRotation( particle.rotation + particle.rotationDiff
						* rotationValue.getScale( percent ) );
		}
		...
		particle.setColor( color[ 0 ], color[ 1 ], color[ 2 ],
				particle.transparency + particle.transparencyDiff
	...
	public void setOffset( float xOffset, float yOffset ) {
		...
		float xDiffLowHalf = (xOffsetValue.getLowMax( )
				- xOffsetValue.getLowMin( )) / 2;
		float yDiffLowHalf = (yOffsetValue.getLowMax( )
				- yOffsetValue.getLowMin( )) / 2;
	...
	public void setLifeValue( float lifeMaxMillis, float lifeMinMillis ) {
		...
		float diffHighHalf = ( lifeValue.getHighMax( ) - lifeValue.getHighMin( ) ) / 2;
		float diffLowHalf = (lifeValue.getLowMax( ) - lifeValue.getLowMin( ) ) / 2;
		lifeValue.setHigh( lifeMaxMillis - diffHighHalf, lifeMaxMillis + diffHighHalf );
		lifeValue.setLow( lifeMinMillis - diffLowHalf, lifeMinMillis + diffHighHalf ); 
	}
	
	public void setEmmisionValue( float highEmits, float lowEmits ) {
		emissionValue.active = true;
		float diffHighHalf = ( emissionValue.getHighMax( ) - emissionValue.getHighMin( ) ) / 2;
		float diffLowHalf = emissionValue.getLowMax( ) - emissionValue.getLowMin( );
		emissionValue.setHigh( highEmits - diffHighHalf, highEmits + diffHighHalf );
		emissionValue.setLow( lowEmits - diffLowHalf, lowEmits + diffHighHalf ); 
	}
	...
	public void flipY( ) {
		angleValue
				.setHigh( -angleValue.getHighMin( ), -angleValue.getHighMax( ) );
		angleValue.setLow( -angleValue.getLowMin( ), -angleValue.getLowMax( ) );

		gravityValue.setHigh( -gravityValue.getHighMin( ),
				-gravityValue.getHighMax( ) );
		gravityValue.setLow( -gravityValue.getLowMin( ),
				-gravityValue.getLowMax( ) );

		windValue.setHigh( -windValue.getHighMin( ), -windValue.getHighMax( ) );
		windValue.setLow( -windValue.getLowMin( ), -windValue.getLowMax( ) );

		rotationValue.setHigh( -rotationValue.getHighMin( ),
				-rotationValue.getHighMax( ) );
		rotationValue.setLow( -rotationValue.getLowMin( ),
				-rotationValue.getLowMax( ) );
	...
	static boolean readBoolean( BufferedReader reader, String name )
			throws IOException {
	...
	static public class GradientColorValue extends ParticleValue {
		...
		public float[ ] getColor( float percent ) {
			...
			float factor = ( percent - startTime )
					/ ( timeline[ endIndex ] - startTime );
			endIndex *= 3;
