...
public class PhysicsTestScreen implements com.badlogic.gdx.Screen {
	...
	public PhysicsTestScreen( ) {
		...
		rope = new Rope( "rope", new Vector2( 8f + ropes.size( ) * 1.0f,
				1.5f ), new Vector2( 16.0f, 32.0f ), 10, null, world );
		ropes.add( rope );
	...
	void buildMoverPlatforms( ) {
		...
		// rope = new Rope( "rope", new Vector2 ( 8f, 1.5f), new Vector2 (
		// 16.0f, 32.0f ), 10, null, world );
	...
	public void render( float deltaTime ) {
		...
		if ( Gdx.input.isKeyPressed( Input.Keys.DPAD_UP ) ) {
			rope = new Rope( "rope", new Vector2( 8f + ropes.size( ) * 1.0f,
					1.5f ), new Vector2( 16.0f, 32.0f ), 10, null, world );
			ropes.add( rope );
			Gdx.app.log( "Ropes", String.valueOf( ropes.size( ) ) );
		}
		...
		Iterator< Rope > itor = ropes.iterator( );
		while ( itor.hasNext( ) ) {
			itor.next( ).update( deltaTime );
		}
